//Basically we have to form a 2D dp[i][j] of number of 1s coming after arr[j][i]
//Then we have to sort all dp[i] arrays. In this way we get an increasing order of number of 1s coming after in each row.
//Now we traverse every row and store the max of dp[i][j]*(n-j). Bcoz j is the current number of rows and all the rows after it will have all 1s after ith column
